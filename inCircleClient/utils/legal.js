  ...
  terms: {
    ...
    sections: [
      ...
      },
      {
        title: "六、圈内 AI 服务",
        paragraphs: [
          "本小程序不自行研发、训练或部署生成式人工智能模型，也不向用户预置或统一分配模型服务。圈子管理者可在符合微信平台规则及适用法律法规的前提下，自主配置其已合法开通且有权使用的第三方模型服务。未配置可用第三方服务时，本小程序不提供相关生成式人工智能问答功能。",
        ...
        bullets: [
          "圈内 AI 受平台总开关控制，并由圈子管理者按圈启用；用户首次使用具体第三方服务商前还需完成单独授权。",
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          "授权登录位置：在你开启“记录精确登录位置”并同意微信位置授权后，我们会在登录或你主动更新时获取一次 GCJ-02 经纬度、设备返回的定位精度和采集时间；启用地址解析服务时还会形成省、市、区及详细地址，用于帮助你识别当前及其他登录设备。",
        ],
      },
      {
      ...
      },
      {
        title: "四、AI 与第三方处理",
        paragraphs: [
          "本小程序不自行研发、训练或部署生成式人工智能模型。平台开放圈内 AI 且圈子管理者自主配置可用的第三方模型服务后，我们会在用户首次使用具体供应商前另行展示供应商名称、域名和隐私政策，并征得单独同意。只有同意后，用户输入、圈级系统提示和当前会话必要上下文才会发送给该供应商。",
      ...
      },
      {
        ...
        bullets: [
          ...
          "圈子管理者自主配置的第三方 AI 服务商：仅在用户单独授权后处理对应 AI 会话内容。",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "登录设备的精确坐标、定位精度、解析地址和采集时间最多保留 90 天，服务会定期清理过期数据。你关闭“记录精确登录位置”时，服务器会立即清除该账号现有设备会话中的精确位置；网络出口地址仍按必要的账号安全与日志期限处理。",
