    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "你可在账号设置中选择是否授权记录登录设备的精确位置。该信息仅用于帮助你识别登录环境；拒绝授权不会阻止登录，且设备定位可能存在偏差或被伪造，不能单独作为身份真实性或账号安全结论。",
        ],
      },
      {
        title: "三、圈子与成员规则",
      ...
      },
      {
        title: "六、圈内 AI 服务",
        paragraphs: [
          "本小程序不自行研发、训练或部署生成式人工智能模型，也不向用户预置或统一分配模型服务。圈主可在符合微信平台规则及适用法律法规的前提下，自主配置其已合法开通且有权使用的第三方模型服务。未配置可用第三方服务时，本小程序不提供相关生成式人工智能问答功能。",
          "本小程序主要提供第三方服务接入配置、圈内权限管理、用户授权、内容安全审核和结果展示等工具能力。具体模型推理及生成内容服务由相应第三方服务商按照其服务协议和隐私政策提供；本小程序仍会按照本协议及《InCircle 隐私政策》履行权限控制和安全治理责任。",
        ],
        bullets: [
          "圈内 AI 受平台总开关控制，并由圈主按圈启用；用户首次使用具体第三方服务商前还需完成单独授权。",
          "用户输入及必要会话上下文会发送至页面明确展示的第三方模型服务商，相关处理同时受该服务商条款与隐私政策约束。",
          "本小程序仅提供文字问答，不提供图片、音频或视频生成，不提供换脸、声音克隆、AI 绘画及面向公众的生成内容发布功能。",
          "AI 输出可能存在错误、遗漏或偏差，仅供参考。医疗、法律、财务、安全等重要决定应咨询具备资质的专业人士。",
          "圈内 AI 不会自动执行圈内操作。用户不得利用第三方模型服务生成或传播违法、侵权和有害内容。",
        ],
      },
      {
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          "设备与安全信息：设备型号与系统环境、登录时间、网络出口地址、接口错误、令牌状态及必要运行日志，用于展示登录设备、防攻击、限流、故障排查和审计，不用于建立商业广告画像。",
          "授权登录位置：在你开启“记录精确登录位置”并同意微信位置授权后，我们会在登录或你主动更新时获取一次 GCJ-02 经纬度、设备返回的定位精度和采集时间；启用地址解析服务时还会形成省、市、区及详细地址，用于帮助你识别当前及其他登录设备。在账号安全、投诉处理或问题核查所必需时，经严格授权的安全管理人员可查看精确坐标、定位精度、解析地址和采集时间。",
        ],
      },
      {
        ...
        bullets: [
          ...
          "聚会位置：仅在你主动创建地点投票或约局并选择位置时调用微信位置选择能力，保存所选地点名称、地址及坐标。",
          "登录位置：仅在登录定位开关开启时，于账号登录或你点击“更新本机位置”后调用 wx.getLocation 获取一次精确位置。我们不在后台运行，不持续定位，也不记录移动轨迹；拒绝或定位失败不会影响账号登录和其他基础功能，但最近登录列表将无法显示本次授权位置。",
          "精确位置信息通常属于敏感个人信息。处理该信息的必要性在于让你区分真实设备位置与微信代理网络出口，帮助发现异常登录环境；可能影响是位置被用于推断你当时所在区域。我们仅在微信单独授权后处理，你可以拒绝或随时关闭并清除已有记录。",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "圈主可按产品规则管理成员与业务，但不能查看其他成员未举报的 AI 私聊正文或登录设备位置。经严格授权的安全管理人员仅可在账号安全、投诉处理、问题核查或履行法定义务所必需时访问相关信息，并接受权限控制与操作审计。",
        ],
      },
      {
        title: "四、AI 与第三方处理",
        paragraphs: [
          "本小程序不自行研发、训练或部署生成式人工智能模型。平台开放圈内 AI 且圈主自主配置可用的第三方模型服务后，我们会在用户首次使用具体供应商前另行展示供应商名称、域名和隐私政策，并征得单独同意。只有同意后，用户输入、圈级系统提示和当前会话必要上下文才会发送给该供应商。",
      ...
      },
      {
        ...
        bullets: [
          "微信平台：用于身份登录、头像昵称选择、位置选择与授权定位、小程序码、订阅通知及内容安全等能力。",
          "腾讯位置服务：仅在服务器启用详细地址解析且你已授权登录定位时，将本次 GCJ-02 经纬度发送至腾讯位置服务进行逆地理编码，以获得省、市、区和详细地址；不会随坐标发送账号密码、昵称、手机号或 AI 对话内容。",
          "云服务器与数据库服务：用于托管接口、数据库、上传媒体、日志与备份，并按必要范围处理数据。",
          "圈主自主配置的第三方 AI 服务商：仅在用户单独授权后处理对应 AI 会话内容。",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "登录设备的精确坐标、定位精度、解析地址和采集时间最多保留 90 天，且仅在账号安全、投诉处理或问题核查所必需时向经严格授权的安全管理人员开放，服务会定期清理过期数据。你关闭“记录精确登录位置”时，服务器会立即清除该账号现有设备会话中的精确位置；网络出口地址仍按必要的账号安全与日志期限处理。",
      ...
      },
      {
        ...
        bullets: [
          ...
          "设备定位结果可能受信号、系统权限、模拟定位或终端环境影响。我们将其作为用户可查看的辅助安全信息，不把 GPS 位置单独作为不可伪造的登录认证因素。",
        ],
        paragraphs: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "撤回授权：可在账号设置中关闭精确登录定位并清除现有设备位置，也可在微信系统设置中关闭位置、相册等权限；撤回不影响此前基于授权进行的合法处理。",
