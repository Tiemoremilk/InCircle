...
- 独立账号设置页集中管理登录保护、最近设备、主题、协议、退出和注销；超管可从同一页面进入管理中心。
- 默认登录时校验原绑定微信；用户输入当前密码确认后可关闭，关闭后仅校验账号密码，原微信绑定关系不变。
- 每个小程序安装实例对应一条设备会话；同一设备再次登录会覆盖最近时间并轮换令牌，其他设备可单独退出，当前设备不能被误踢。
- 精确登录定位默认关闭；用户主动开启并完成微信授权后，可在登录或主动更新时记录一次 GCJ-02 位置。最近登录同时区分显示“授权定位”和“网络出口”，拒绝定位不影响登录，也不会持续追踪。
- 精确登录位置最多保留 90 天；关闭账号设置中的定位开关会立即清除该账号已有设备位置。
...
- 维护圈子名称、公告、介绍和入圈信息。
- 生成严格区分大小写的 8 位邀请码以及微信官方小程序入圈码；分享与二维码使用独立安全令牌，不把特殊字符邀请码直接放入链接。
- 圈主或超管可以确认后更换邀请码；更换后旧邀请码、旧分享入口和旧二维码立即失效，新二维码按需生成。
- 历史版本中直接携带邀请码的分享链接和二维码不再兼容；分享与扫码仅识别当前安全令牌，邀请码仍可在加入页手工输入。
- 查看当前圈成员和成员详情。
...
- 系统资料只描述成员可见业务，不向普通用户暴露超管权限和内部实现。
...
| 超管 | 具有全局账号权限；继承原圈内管理能力，并管理全平台圈子、用户、日志和 AI 举报 | 不写入圈成员角色，执行管理操作不要求存在圈成员关系，也不改变圈子所有权 |

技术实现将圈内角色与全局账号权限分开保存：

- 圈成员关系中的 <code>role</code> 只允许“圈主”或“成员”。
- 用户表中的 <code>is_super_admin = true</code> 表示该账号具有“超管”权限。
- 原圈内管理权限统一由“圈主或超管”获得；超管无需加入目标圈子，也不会因此成为圈主。

历史圈成员关系中的旧管理角色会在迁移时降为“成员”，只有圈子所有者保留“圈主”；旧角色不会被提升为超管账号。
...
| <code>TENCENT_MAP_KEY</code> | 可选，严禁提交 | 服务端将已授权的 GCJ-02 坐标解析为省市区和详细地址；未配置时仍保存坐标与精度 |
...
<code>app.json</code> 声明 <code>chooseLocation</code> 和 <code>getLocation</code>：前者用于地点投票和约局地点选择，后者用于用户授权后的登录设备位置记录。

使用位置能力前，还必须在微信公众平台完成以下配置：

1. 在“开发 → 开发管理 → 接口设置”中申请并开通地理位置接口；该接口只对微信允许的主体和服务类目开放。
2. 在“小程序用户隐私保护指引”中为 <code>getLocation</code> 声明“收集你的位置信息”，为 <code>chooseLocation</code> 声明“收集你选择的位置信息”，如实填写用途并确认线上版本已生效。

用户主动开启或更新登录位置时，客户端在点击事件中直接调用 <code>wx.getLocation</code>，由微信展示官方隐私确认和原生位置授权，不提前调用 <code>wx.authorize</code>。用户拒绝小程序位置 Scope 后才通过 <code>openSetting</code> 恢复；微信自身缺少系统定位权限时则进入 <code>wx.openAppAuthorizeSetting</code>，两类设置入口不可混用。登录后的自动采集只在隐私、Scope、手机定位和微信系统权限均已就绪时静默执行，不弹授权窗口、不阻塞登录。
...
| 分组 | action |
| --- | --- |
| 账号 | <code>incircleSession</code>、账号登录/注册/绑定、重置/修改密码、账号设置、微信登录校验、精确登录定位设置与更新、设备会话退出、当前设备退出和注销 |
...
服务端每 2 秒发送心跳。requestId 用于幂等和断线恢复，客户端按 offset 去重已经展示的内容。生成任务同时维护 30 秒数据库租约；进程异常退出后，过期任务会转为失败而不会永久停在“正在思考”。正常部署收到退出信号时会先停止当前生成并保存明确终态。

## 圈内 AI

InCircle 不自行研发、训练或部署生成式人工智能模型，也不预置统一模型服务。圈主或超管需自行接入其合法开通并有权使用的第三方模型服务；未配置可用服务时，圈内 AI 不提供问答能力。

超管后台提供总开关。关闭后，所有圈子的 AI 悬浮入口、圈子设置入口和后端 AI 接口同时停用；已有供应商、模型、授权和会话数据保留，重新开放后仍按各圈原有开关与配置恢复。
...
自定义 Base URL 只允许圈主或超管配置，所有地址必须使用 HTTPS。
...
- 系统不会通过发送 128K 或 1M 巨量文本探测上下文窗口；未知上下文需依赖供应商元数据、数据库能力目录或圈主、超管手动补充。
...
- 实际输出取圈子额度、模型已知输出能力和上下文剩余额度的最小值。
- 输出能力未知时先按圈子额度调用；只有供应商在零输出阶段明确拒绝输出参数，才安全回退到 8192 Token。
- 测试连接在具体模型上执行，固定短提示和能力探测不计入成员额度，也不保存为聊天记录。
- 模型可启用、停用、归档和设为默认。
- 历史消息保存模型和供应商快照。
...
- 当前授权用于实时权限判断；每个供应商隐私版本的确认另存不可覆盖的审计记录。账号删除后只保留随机主体标识、版本、供应商快照和确认时间。
...
- <code>incircle_account_sessions</code>：按安装实例保存哈希设备键、最近登录环境、网络出口、经授权的精确登录位置、令牌版本和撤销状态；精确位置定期按 90 天留存策略清理。
...
当前种子以 <code>models.dev</code> 社区目录为起点，并保留供应商文档地址。它用于扩大可识别范围，不被描述为官方事实，也不保证覆盖世界上所有模型。生产服务不会抓取互联网；供应商实时返回的元数据和圈主、超管手动值始终优先。
...
生成后应审查目录差异和异常能力值，再提交新的版本号。已经导入数据库的版本号绝不能复用：同版本、同 checksum 的部署是无写入操作；同版本但文件内容变化会直接终止迁移。新版本会在一个事务中导入，数据库中 <code>source_kind = manual</code> 的人工记录以及 <code>official/verified</code> 高可信记录不会被社区数据降级覆盖。需要单独重试目录导入时可运行 <code>npm run db:catalog:import</code>。
...
迁移器会创建迁移记录表、执行幂等 Schema、按文件名应用增量迁移、校验已执行文件的 SHA-256、导入当前版本的模型能力目录，并补齐系统资料。
...
| <code>-SuperAdminOpenids</code> | 空 | 超管账号 OpenID |
...
8. 在迁移事务完成后导入新版本模型能力目录；同版本部署不改写目录数据。
9. 仅在法律资料字段缺失时用服务器配置完成首次初始化；已有数据库值保持不变。
10. 迁移成功后启动 API。
11. 最多等待约 60 秒检查 <code>/health</code>，并确认宿主机端口只绑定到 <code>127.0.0.1</code>。
12. 失败时输出容器状态和最近日志。
...
API Docker 日志单文件最大 10 MiB，最多 3 个文件。操作日志保存在 PostgreSQL，可由超管管理。日志不应写入密码、AppSecret、AI API Key 或普通聊天正文。
...
- 设备安装标识只以 SHA-256 哈希落库；网络出口取自受信任反向代理后的请求 IP，不将微信代理 IP 当作用户实际所在地。
- 精确登录位置默认关闭，仅在用户主动开启并通过微信位置授权后，于登录或主动更新时采集一次；登录自动采集不会主动申请权限，不后台定位、不记录轨迹，失败或拒绝不阻断登录。
- 配置 <code>TENCENT_MAP_KEY</code> 后，服务端只向固定腾讯逆地理编码接口发送本次 GCJ-02 坐标，不发送账号密码、昵称、手机号或会话正文；密钥不进入客户端和日志。
- 定位结果可能存在误差或被终端伪造，只作为用户可见的辅助安全信息，不单独参与身份认证。
- 登录尝试 10 分钟最多 8 次。
- 封禁、解绑和关键状态变化撤销旧令牌。
- 生产环境隐藏未知内部异常。
...
- <code>inCircleClient/config/backend.js</code>
- 精确文件名 <code>.env</code>
- <code>server/uploads/</code>
- <code>incircle-server.tar.gz</code>
- <code>scripts/deploy-server.ps1</code>
- Windows 系统元数据
